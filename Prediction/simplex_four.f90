        subroutine simplex_four(nAtoms,con_mat,ncount,filename)

        implicit none
        integer,dimension(nAtoms,nAtoms) :: con_mat
        integer,dimension(4) :: a
        integer,dimension(3) :: scl,ix,tempa
        integer,dimension(4,4) :: new_mat
        integer,dimension(4,4) :: upd_mat
       ...
       &                                  flagm,smc)
                      if(flagm==0) then
                   write(57,*) i,j,k,l,0,0
                   ncount=ncount+1
                   else
                   write(58,*) i-1,j-1,k-1,l-1
                   end if
                   end if
                   end if
